    ...
    for tag, val in tag_dict.items():
        ...
        for match in matches:
            if "." in match:
                ...
                if ref_tc not in sent_fix_messages:
                    raise ValueError(f"Test case {ref_tc} not found for placeholder {match}")
                if ref_tag not in sent_fix_messages[ref_tc]:
                    raise ValueError(f"Tag {ref_tag} not found in test case {ref_tc} for placeholder {match}")
                replacement = sent_fix_messages[ref_tc][ref_tag]
            else:
                if match not in current_sent_tags:
                    raise ValueError(f"Tag {match} not found in current message for placeholder {match}")
    ...
    with open(input_file, newline="") as csvfile:
        ...
        for row in reader:
            ...
            for idx, case in enumerate(final_cases, start=1):
                ...
                message_type = case['TagsToUpdate'].get('35', 'UNKNOWN')

                print(f"[{idx}/{len(final_cases)}] Running TestCaseID: {testcase_id}, MessageType: {message_type} ...", end=" ", flush=True)
                ...
                sent_msg_tags = parse_fix(sent_msg)
                ...
                if is_pass:
                    passed += 1
                    log.info(f"Test Case {testcase_id} [PASS]")
                    print("✅ PASS")
                else:
                    failed += 1
                    log.error(f"Test Case {testcase_id} [FAIL] Reason(s): {' | '.join(messages)}")
                    print("❌ FAIL")
