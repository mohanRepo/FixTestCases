...
received_fix_messages = {}   # {TestCaseID: {tag: value}}

# ---- Utility Functions ----

def safe_resolve_placeholders(tag_dict, testcase_id, sent_fix_messages, current_sent_tags, current_received_tags, allow_missing=False):
    resolved = {}
    ...
    for tag, val in tag_dict.items():
        ...
        for match in matches:
            ...
            if match.startswith("OUT."):
                out_parts = match[4:].split(".", 1)
                if len(out_parts) == 2:
                    ref_tc, ref_tag = out_parts
                    replacement = received_fix_messages.get(ref_tc, {}).get(ref_tag)
                else:
                    replacement = current_received_tags.get(out_parts[0])
            elif "." in match:
                ref_tc, ref_tag = match.split(".", 1)
                replacement = sent_fix_messages.get(ref_tc, {}).get(ref_tag)
            else:
                replacement = current_sent_tags.get(match)

            if replacement is None:
                if allow_missing:
                    continue
                else:
                    raise ValueError(f"Missing placeholder: {match}")
...
def expand_test_cases(row: Dict[str, str]) -> (list, str):
    ...
    if multi_tag and multi_values:
        for idx, val in enumerate(multi_values):
            ...
            validate = {tag: values[min(idx, len(values) - 1)] for tag, values in validate_multi_values.items()}
    ...
    return expanded_cases, multi_35_values[1] if multi_35_values and len(multi_35_values) > 1 else None
    ...
    with open(input_file, newline="") as csvfile:
        ...
        for row in all_rows:
            ...
            for case in base_cases:
                ...
                try:
                    resolved_update = safe_resolve_placeholders(case["TagsToUpdate"], testcase_id, sent_fix_messages, {}, {}, allow_missing=True)
                except Exception as e:
                    print(f"❌ Placeholder resolution failed in first pass: {e}")
                    continue

                tag11 = resolved_update.get("11", "")
                if not tag11 or "${" in tag11:
                    tag11 = f"{testcase_id}_{uuid.uuid4().hex[:8]}"
                    resolved_update["11"] = tag11

                resolved_update = safe_resolve_placeholders(case["TagsToUpdate"], testcase_id, sent_fix_messages, resolved_update, {}, allow_missing=False)
                resolved_update["11"] = tag11

                case["TagsToUpdate"] = resolved_update
                case["tag11"] = tag11
            ...
            for idx, case in enumerate(final_cases, start=1):
                ...
                try:
                    resolved_validate = safe_resolve_placeholders(
                        case["TagsToValidate"], testcase_id,
                        sent_fix_messages,
                        case["TagsToUpdate"],
                        received_fix_messages.get(testcase_id, {})
                    )
                except Exception as e:
                ...
                received_fix_messages[testcase_id] = received_tags
