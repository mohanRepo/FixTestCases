...
# ---- Utility Functions ----
def safe_resolve_placeholders(tag_dict, testcase_id, sent_fix_messages, current_sent_tags, allow_missing=False):
    resolved = {}
...
def expand_test_cases(row: Dict[str, str]) -> (list, str):
    ...
    for part in update_parts:
        ...
        if tag == "35" and MULTI_VAL_DELIMITER in value:
            full_35_sequence = value.split(MULTI_VAL_DELIMITER)
            if len(full_35_sequence) > 2:
                expanded_cases = []
                for i in range(1, len(full_35_sequence)):
                    clone = row.copy()
                    clone["TagsToUpdate"] = row["TagsToUpdate"].replace(value, f"{full_35_sequence[0]}~{full_35_sequence[i]}")
                    cases, _ = expand_test_cases(clone)
                    expanded_cases.extend(cases)
                return expanded_cases, None
            else:
                multi_35_values = full_35_sequence
    ...
    with open(input_file, newline="") as csvfile:
        ...
        for row in all_rows:
            ...
            for case in base_cases:
                ...
                resolved_update = safe_resolve_placeholders(case["TagsToUpdate"], testcase_id, sent_fix_messages, resolved_update)
                resolved_update["11"] = tag11
            ...
            for idx, case in enumerate(final_cases, start=1):
                ...
                try:
                    resolved_validate = safe_resolve_placeholders(case["TagsToValidate"], testcase_id, sent_fix_messages, case["TagsToUpdate"])
                except Exception as e:
